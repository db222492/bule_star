...
import static com.xinzeyijia.houselocks.util.LogUtils.*;
...
public final class AppUtil {
    ...
    public static boolean isAppAlive(@NonNull String packageName) {
        ...
        for (int i = 0; i < processInfos.size(); i++) {
            if (processInfos.get(i).processName.equals(packageName)) {
                logi(TAG, String.format("AppAliveInfo ========> App %s is running", packageName));
                return true;
            }
        }
        logi(TAG, String.format("AppAliveInfo ========> App %s has been killed", packageName));
        return false;
    }
    ...
    public int getAppStatus(@NonNull String packageName) {
        ...
        if (taskInfos.get(0).topActivity.getPackageName().equals(packageName)) {
            logi(TAG, String.format("AppStatusInfo ========> App %s is running onForeground", packageName));
            return APP_FORE;
        } else {
            // 判断 App 是否在堆栈中
            for (ActivityManager.RunningTaskInfo info : taskInfos) {
                if (info.topActivity.getPackageName().equals(packageName)) {
                    logi(TAG, String.format("AppStatusInfo ========> App %s is running onBackground", packageName));
                    return APP_BACK;
                }
            }
            logi(TAG, String.format("AppStatusInfo ========> App %s has been killed", packageName));
            return APP_DEAD;
        }
    }
    ...
    public static boolean installAppSilent(@NonNull String filePath) {
        ...
        try {
            ...
            logd("", "install msg is " + msg);
            // 如果执行结果中包含 Failure 字样就认为是安装失败，否则就认为安装成功
            if (!msg.contains("Failure")) {
                result = true;
            }
        } catch (Exception e) {
            loge("InstallAppSilent", "InstallException ========> " + e.getMessage());
        } finally {
            try {
    ...
    public static String getAppSignatureSHA1() {
        try {
            ...
            logi(TAG, String.format("AppSHA1 ========> SHA1 is %s", sha1));
            return sha1;
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
