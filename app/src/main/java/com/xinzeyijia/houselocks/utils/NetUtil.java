...
import com.xinzeyijia.houselocks.model.constant.NetworkType;
...
import static com.xinzeyijia.houselocks.util.LogUtils.logd;
...
public final class NetUtil {
     ...
     * <p>需要异步 ping，如果 ping 不通就说明网络不可用</p>
     *
     * @param ip ip location_icon（自己服务器 ip），如果为空，ip 为阿里巴巴公共 ip
     * @return {@code true}: 可用<br>{@code false}: 不可用
     */
    public static boolean isAvailableByPing(String ip) {
        ...
        if (result.successMsg != null) {
            logd("NetUtil", "isAvailableByPing() called" + result.successMsg);
        }
        if (result.errorMsg != null) {
            logd("NetUtil", "isAvailableByPing() called" + result.errorMsg);
        }
        return ret;
    }
    ...
    /**
     * 通过 wifi 获取本地 IP location_icon
     *
     * @return IP location_icon
     */
