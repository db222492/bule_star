package com.xinzeyijia.houselocks.model.exception;
...
import static com.xinzeyijia.houselocks.util.LogUtils.logw;
import static com.xinzeyijia.houselocks.util.sdutil.Path.*;
    ...
    public void writeLogToSDCard(String log) {
        if (LOG_DEBUG){
            try {
                ...
                if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    if (LOG_DEBUG) {
                        logw( "sdcard unmounted,skip dump exception");
                        return;
                    }
                }
