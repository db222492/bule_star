package com.xinzeyijia.houselocks.handler;
...
import static com.xinzeyijia.houselocks.util.sdutil.Path.FILE_NAME;
import static com.xinzeyijia.houselocks.util.sdutil.Path.FILE_NAME_SUFFIX;
import static com.xinzeyijia.houselocks.util.sdutil.Path.LOG_NAME;
import static com.xinzeyijia.houselocks.util.sdutil.Path.SD_CARD_EXCEPTION_PATH;
import static com.xinzeyijia.houselocks.util.sdutil.Path.SD_CARD_LOG_PATH;
...
public class XCrashHandlerUtils implements Thread.UncaughtExceptionHandler {
    ...
    private static final class SingleTon{
        private static final XCrashHandlerUtils X_CRASH_HANDLER_UTILS = new XCrashHandlerUtils();
    }
    //程序的Context对象
    ...
    public static XCrashHandlerUtils getInstance() {
        return SingleTon.X_CRASH_HANDLER_UTILS;
    }
...
//        uploadExceptionToServer();
    ...
    public void dumpExceptionToSDCard(Throwable e) throws IOException {
        ...
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            if (LOG_DEBUG) {
                ...
                writeLogToSDCard("sdcard unmounted,skip dump exception");
                return;
            }
        }
        File dir = new File(SD_CARD_EXCEPTION_PATH);
        //如果目录下没有文件夹，就创建文件夹
        if (!dir.exists()) {
            dir.mkdirs();
        }
        ...
        //在定义的Crash文件夹下创建文件
        File file = new File(SD_CARD_EXCEPTION_PATH + FILE_NAME + time + FILE_NAME_SUFFIX);
     ...
     * @param
     */
    public void writeLogToSDCard(String log) {
        try {

            File dir = new File(SD_CARD_LOG_PATH);
            //如果目录下没有文件夹，就创建文件夹
            if (!dir.exists()) {
                dir.mkdirs();
            }
            ...
            String time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(current));
            //在定义的Crash文件夹下创建文件
            File file = new File(SD_CARD_LOG_PATH + LOG_NAME + time + FILE_NAME_SUFFIX);
